...
public class WorkspaceIssueController {
    ...
    private final IWorkspaceIssueService workspaceIssueService;
    ...
    @GetMapping("/{id}")
    public Result<WorkspaceIssueDetailVO> getById(@PathVariable Long id) {
        try {
            log.info("[查询事项] ID: {}", id);
            WorkspaceIssueDetailVO issueDetail = workspaceIssueService.getIssueDetailById(id);
            if (issueDetail == null) {
    ...
    @RequiresPermission("workspace:issue:view")
    @PostMapping("/page")
    public Result<PageVO<WorkspaceIssuePageVO>> page(@RequestBody WorkspaceIssueQueryRequest request) {
        log.info("[分页查询事项] 查询参数: {}", request);
        try {
            PageVO<WorkspaceIssuePageVO> page = workspaceIssueService.pageQuery(request);
