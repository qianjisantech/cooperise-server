...
import com.dcp.common.request.WorkSpaceIssueRequest;
import com.dcp.common.request.WorkspaceIssueQueryRequest;
import com.dcp.common.util.IssueNoGenerator;
import com.dcp.common.vo.PageVO;
import com.dcp.common.vo.WorkspaceIssueDetailVO;
import com.dcp.common.vo.WorkspaceIssuePageVO;
import com.dcp.common.vo.WorkspaceIssueVO;
import com.dcp.entity.Space;
import com.dcp.entity.WorkspaceIssue;
import com.dcp.mapper.WorkspaceIssueMapper;
import com.dcp.service.ISpaceService;
import com.dcp.service.IWorkspaceIssueService;
...
public class WorkspaceIssueServiceImpl extends ServiceImpl<WorkspaceIssueMapper, WorkspaceIssue> implements IWorkspaceIssueService {

    private final ISpaceService spaceService;
    private final com.dcp.service.ISpaceMemberService spaceMemberService;
    ...
    public void createIssue(WorkSpaceIssueRequest request) {
        WorkspaceIssue workspaceIssue = new WorkspaceIssue();

        // 1. 处理空间信息
        if (request.getSpace() != null) {
            WorkSpaceIssueRequest.SpaceDTO space = request.getSpace();
    ...
    @Override
    public WorkspaceIssueDetailVO getIssueDetailById(Long id) {
        // 1. 查询事项
        WorkspaceIssue issue = getById(id);
        if (issue == null) {
            return null;
        }
        ...
        // 3. 创建详情VO对象
        WorkspaceIssueDetailVO detailVO = new WorkspaceIssueDetailVO();
    ...
    @Override
    public PageVO<WorkspaceIssuePageVO> pageQuery(WorkspaceIssueQueryRequest request) {
        // 1. 创建分页对象
        Page<WorkspaceIssue> page = new Page<>(request.getCurrent(), request.getSize());

        // 2. 构建查询条件
        LambdaQueryWrapper<WorkspaceIssue> queryWrapper = buildQueryWrapper(request);

        // 3. 执行分页查询
        Page<WorkspaceIssue> entityPage = page(page, queryWrapper);

        // 4. 转换 Entity 到 VO
        List<WorkspaceIssuePageVO> voList = entityPage.getRecords().stream()
                .map(this::convertToVO)
                .collect(Collectors.toList());

        // 5. 构建 PageVO
        PageVO<WorkspaceIssuePageVO> pageVO = new PageVO<>();
     ...
     * @param space 事项对象
     */
    private String generateIssueNo(WorkSpaceIssueRequest.SpaceDTO space) {
     ...
     * @return 查询条件包装器
     */
    private LambdaQueryWrapper<WorkspaceIssue> buildQueryWrapper(WorkspaceIssueQueryRequest request) {
        LambdaQueryWrapper<WorkspaceIssue> queryWrapper = new LambdaQueryWrapper<>();
        ...
        if (userContext != null && userContext.getUserId() != null) {
            ...
            if (userSpaceIds != null && !userSpaceIds.isEmpty()) {
                // 只查询用户所在空间的事项
                queryWrapper.in(WorkspaceIssue::getSpaceId, userSpaceIds);
            } else {
                // 用户不在任何空间中,返回空结果
                queryWrapper.eq(WorkspaceIssue::getId, -1L);
            }
        }

        // 事项单号（精确匹配）
        if (StringUtils.isNotBlank(request.getIssueNo())) {
            queryWrapper.eq(WorkspaceIssue::getIssueNo, request.getIssueNo());
        }

        // 标题（模糊查询）
        if (StringUtils.isNotBlank(request.getSummary())) {
            queryWrapper.like(WorkspaceIssue::getSummary, request.getSummary());
        }

        // 默认按创建时间倒序
        queryWrapper.orderByDesc(WorkspaceIssue::getCreateTime);
     ...
     * @return PageVO对象
     */
    private WorkspaceIssuePageVO convertToVO(WorkspaceIssue entity) {
        WorkspaceIssuePageVO vo = new WorkspaceIssuePageVO();
    ...
    public List<Map<String, Object>> searchIssues(String keyword) {
        LambdaQueryWrapper<WorkspaceIssue> wrapper = new LambdaQueryWrapper<>();
        ...
        if (userContext != null && userContext.getUserId() != null) {
            ...
            if (userSpaceIds != null && !userSpaceIds.isEmpty()) {
                // 只搜索用户所在空间的事项
                wrapper.in(WorkspaceIssue::getSpaceId, userSpaceIds);
            } else {
                // 用户不在任何空间中,返回空结果
                return List.of();
            }
        ...
        // 支持事项单号或标题的模糊搜索
        wrapper.and(w -> w.like(WorkspaceIssue::getIssueNo, keyword)
                          .or()
                          .like(WorkspaceIssue::getSummary, keyword));

        // 只返回未删除的
        wrapper.eq(WorkspaceIssue::getIsDeleted, 0);

        // 按创建时间倒序
        wrapper.orderByDesc(WorkspaceIssue::getCreateTime);

        // 限制返回10条
        wrapper.last("LIMIT 10");

        List<WorkspaceIssue> issues = baseMapper.selectList(wrapper);
