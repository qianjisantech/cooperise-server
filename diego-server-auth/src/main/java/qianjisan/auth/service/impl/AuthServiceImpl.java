package qianjisan.auth.service.impl;
...
import com.qianjisan.enterprise.mapper.UserCompanyMapper;
import com.qianjisan.common.vo.CompanyVo;
import qianjisan.auth.service.IAuthService;
import qianjisan.auth.service.IVerificationCodeService;
import qianjisan.auth.vo.LoginResponseVO;
...
public class AuthServiceImpl implements IAuthService {
    ...
    public SysUserProfileVO getUserProfile(Long userId) {
        ...
        if (isAdmin) {
            ...
            // admin 用户空间信息（当前去除 space 相关逻辑，返回空集合）
            profile.setSpaces(new ArrayList<>());
            profile.setSpaceIds(new Long[0]);
            ...
            // 普通用户空间信息（当前去除 space 相关逻辑，返回空集合）
            profile.setSpaces(new ArrayList<>());
            profile.setSpaceIds(new Long[0]);
        ...
        // --- 新增：查询用户所属企业（优先从 user_company 表查找）
        // TODO: 暂时注释掉企业功能，待解决依赖注入问题后再启用
        /*
        try {
            List<Long> companyIds = userCompanyMapper.selectCompanyIdsByUserId(userId);
            if (companyIds != null && !companyIds.isEmpty()) {
                List<Company> companies = companyService.listByIds(companyIds);
                List<CompanyVo> companyVos = new java.util.ArrayList<>();
                for (Company c : companies) {
                    CompanyVo cv = new CompanyVo();
                    cv.setId(c.getId());
                    cv.setCompanyCode(c.getCompanyCode());
                    cv.setCompanyName(c.getCompanyName());
                    cv.setShortName(c.getShortName());
                    cv.setCreditCode(c.getCreditCode());
                    cv.setDescription(c.getDescription());
                    cv.setContactPerson(c.getContactPerson());
                    cv.setContactPhone(c.getContactPhone());
                    cv.setContactEmail(c.getContactEmail());
                    cv.setAddress(c.getAddress());
                    cv.setStatus(c.getStatus());
                    companyVos.add(cv);
                }
                profile.setCompanies(companyVos);
                profile.setCompanyIds(companyIds.toArray(new Long[0]));
            } else {
                profile.setCompanies(new ArrayList<>());
                profile.setCompanyIds(new Long[0]);
            }
        } catch (Exception e) {
            log.warn("[AuthService] 查询用户企业失败，用户ID: {}, 错误: {}", userId, e.getMessage());
            profile.setCompanies(new ArrayList<>());
            profile.setCompanyIds(new Long[0]);
        }
        */
